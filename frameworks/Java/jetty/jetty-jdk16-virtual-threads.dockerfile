...
ENV USE_VIRTUAL_THREADS true
CMD ["java", "-XX:+UseNUMA", "-XX:+UseParallelGC", "-jar", "target/jetty-example-0.1-jar-with-dependencies.jar", "loom"]
#CMD ["java", "-XX:+UseNUMA", "-XX:+UseParallelGC", "-Djdk.tracePinnedThreads=full", "-jar", "target/jetty-example-0.1-jar-with-dependencies.jar"]
